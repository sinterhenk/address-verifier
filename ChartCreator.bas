...
' ============================================================
' BUILD + SHOW THE FORM AT RUNTIME (no .frm file needed)
'
' Creates a temporary UserForm via VBProject, adds controls
' and event code, shows it, then cleans up.
'
' Requires: Trust access to the VBA project object model
'   (File > Options > Trust Center > Macro Settings)
' If that is off, falls back to an InputBox workflow.
' ============================================================
Private Sub ShowFormViaVBProject()
    ...
    ' Remove any leftover temp form from a previous run
    On Error Resume Next
    vbProj.VBComponents.Remove vbProj.VBComponents("_TmpChartFrm")
    On Error GoTo Fallback

    ' Add a new UserForm component (type 3 = vbext_ct_MSForm)
    Set vbComp = vbProj.VBComponents.Add(3)
    vbComp.Name = "_TmpChartFrm"

    ' Set form properties via the designer
    With vbComp.Properties
        .Item("Caption") = "Chart Creator"
        .Item("Width") = 380
        .Item("Height") = 440
        .Item("BackColor") = RGB(245, 245, 248)
    End With

    ' Add controls to designer
    Dim dsgn As Object
    Set dsgn = vbComp.Designer

    Dim y As Long
    y = 10

    ' Data Range
    AddFormLabel dsgn, "lblRange", "Data Range:", 10, y, 100, 15
    AddFormTextBox dsgn, "txtRange", 115, y, 165, 18
    AddFormButton dsgn, "btnPickRange", "...", 285, y, 25, 18
    y = y + 28

    ' Chart Type
    AddFormLabel dsgn, "lblType", "Chart Type:", 10, y, 100, 15
    AddFormCombo dsgn, "cboChartType", 115, y, 175, 18
    y = y + 28

    ' Chart Title
    AddFormLabel dsgn, "lblTitle", "Chart Title:", 10, y, 100, 15
    AddFormTextBox dsgn, "txtTitle", 115, y, 175, 18
    y = y + 28

    ' Placement
    AddFormLabel dsgn, "lblPlace", "Placement:", 10, y, 100, 15
    AddFormCombo dsgn, "cboPlacement", 115, y, 175, 18
    y = y + 28

    ' Color Scheme
    AddFormLabel dsgn, "lblColor", "Color Scheme:", 10, y, 100, 15
    AddFormCombo dsgn, "cboColor", 115, y, 175, 18
    y = y + 28

    ' Size
    AddFormLabel dsgn, "lblSize", "Size (pts):", 10, y, 100, 15
    AddFormLabel dsgn, "lblW", "W:", 115, y, 15, 15
    AddFormTextBox dsgn, "txtW", 132, y, 65, 18
    AddFormLabel dsgn, "lblH", "H:", 204, y, 15, 15
    AddFormTextBox dsgn, "txtH", 220, y, 65, 18
    y = y + 28

    ' Checkboxes
    AddFormCheckBox dsgn, "chkHeaders", "First row/column = headers", 10, y, 240, 18
    y = y + 22
    AddFormCheckBox dsgn, "chkLegend", "Show legend", 10, y, 180, 18
    y = y + 22
    AddFormCheckBox dsgn, "chkLabels", "Show data labels", 10, y, 180, 18
    y = y + 30

    ' Buttons
    AddFormButton dsgn, "btnCreate", "Create Chart", 80, y, 100, 24
    AddFormButton dsgn, "btnCancel", "Cancel", 195, y, 80, 24

    ' Inject event-handler code into the form module
...
Fallback:
    ' Clean up partial form if it exists
    On Error Resume Next
    vbProj.VBComponents.Remove vbProj.VBComponents("_TmpChartFrm")
    On Error GoTo 0
...
' ============================================================
' VBA code string injected into the runtime-created UserForm
' ============================================================
Private Function BuildFormCode() As String
    ...
    c = c & "  Dim types As Variant" & N
    c = c & "  types = Array(""Column (Vertical)"",""Bar (Horizontal)"",""Line""," & _
            """Line with Markers"",""Pie"",""Doughnut"",""Area"",""Scatter""," & _
            """Scatter with Lines"",""Stacked Column"",""Stacked Bar"",""Radar"")" & N
    c = c & "  Dim t As Variant" & N
    c = c & "  For Each t In types: cboChartType.AddItem t: Next t" & N
    c = c & "  cboChartType.ListIndex = 0" & N
    c = c & "  cboPlacement.AddItem ""Right of data""" & N
    c = c & "  cboPlacement.AddItem ""Below data""" & N
    c = c & "  cboPlacement.AddItem ""New sheet""" & N
    c = c & "  cboPlacement.ListIndex = 0" & N
    c = c & "  Dim schemes As Variant" & N
    c = c & "  schemes = Array(""Canva"",""Default"",""Office"",""Vivid"",""Pastel"",""Greyscale"",""Dark"")" & N
    c = c & "  Dim sc As Variant" & N
    c = c & "  For Each sc In schemes: cboColor.AddItem sc: Next sc" & N
    c = c & "  cboColor.ListIndex = 0" & N
    c = c & "  txtW.Text = ""480""" & N
    c = c & "  txtH.Text = ""300""" & N
    c = c & "  chkHeaders.Value = True" & N
    c = c & "  chkLegend.Value = True" & N
    c = c & "  chkLabels.Value = False" & N
    c = c & "  On Error Resume Next" & N
    c = c & "  txtRange.Text = Selection.Address" & N
    c = c & "  On Error GoTo 0" & N
    c = c & "End Sub" & N & N

    ' --- Pick Range button ---
    c = c & "Private Sub btnPickRange_Click()" & N
    c = c & "  Dim rng As Range" & N
    c = c & "  On Error Resume Next" & N
    c = c & "  Set rng = Application.InputBox(""Select your data range:"",""Select Range"",Type:=8)" & N
    c = c & "  On Error GoTo 0" & N
    c = c & "  If Not rng Is Nothing Then txtRange.Text = rng.Address(External:=True)" & N
    c = c & "End Sub" & N & N

    ' --- Create button ---
    c = c & "Private Sub btnCreate_Click()" & N
    c = c & "  If Trim(txtRange.Text) = """" Then" & N
    c = c & "    MsgBox ""Please select a data range."", vbExclamation, ""Chart Creator""" & N
    c = c & "    Exit Sub" & N
    c = c & "  End If" & N
    c = c & "  Dim rng As Range" & N
    c = c & "  On Error Resume Next" & N
    c = c & "  Set rng = Application.Range(txtRange.Text)" & N
    c = c & "  On Error GoTo 0" & N
    c = c & "  If rng Is Nothing Then" & N
    c = c & "    MsgBox ""Invalid range."", vbExclamation, ""Chart Creator""" & N
    c = c & "    Exit Sub" & N
    c = c & "  End If" & N
    c = c & "  If Not IsNumeric(txtW.Text) Or CDbl(txtW.Text) < 50 Then" & N
    c = c & "    MsgBox ""Width must be >= 50."", vbExclamation, ""Chart Creator""" & N
    c = c & "    Exit Sub" & N
    c = c & "  End If" & N
    c = c & "  If Not IsNumeric(txtH.Text) Or CDbl(txtH.Text) < 50 Then" & N
    c = c & "    MsgBox ""Height must be >= 50."", vbExclamation, ""Chart Creator""" & N
    c = c & "    Exit Sub" & N
    c = c & "  End If" & N
    c = c & "  Dim pl As String" & N
    c = c & "  Select Case cboPlacement.ListIndex" & N
    c = c & "    Case 0: pl = ""Right""" & N
    c = c & "    Case 1: pl = ""Below""" & N
    c = c & "    Case 2: pl = ""NewSheet""" & N
    c = c & "  End Select" & N
    c = c & "  ChartCreator.CreateChart rng, " & _
            "ChartCreator.ChartTypeFromName(cboChartType.Text), " & _
            "Trim(txtTitle.Text), CBool(chkHeaders.Value), pl, " & _
            "CBool(chkLegend.Value), CBool(chkLabels.Value), " & _
            "cboColor.Text, CDbl(txtW.Text), CDbl(txtH.Text)" & N
    c = c & "  Unload Me" & N
    c = c & "End Sub" & N & N
...
' ============================================================
' FALLBACK: InputBox-based workflow (no VBProject access needed)
' ============================================================
...
' ============================================================
' Designer helpers - add controls to a UserForm designer
' ============================================================
Private Sub AddFormLabel(dsgn As Object, n As String, cap As String, _
    x As Long, y As Long, w As Long, h As Long)
    Dim ctl As Object
    Set ctl = dsgn.Controls.Add("Forms.Label.1")
    With ctl
        .Name = n: .Caption = cap
...
Private Sub AddFormTextBox(dsgn As Object, n As String, _
    x As Long, y As Long, w As Long, h As Long)
    Dim ctl As Object
    Set ctl = dsgn.Controls.Add("Forms.TextBox.1")
    With ctl
        .Name = n
        .Left = x: .Top = y: .Width = w: .Height = h
        .Font.Size = 9
...
Private Sub AddFormCombo(dsgn As Object, n As String, _
    x As Long, y As Long, w As Long, h As Long)
    Dim ctl As Object
    Set ctl = dsgn.Controls.Add("Forms.ComboBox.1")
    With ctl
        .Name = n
        .Left = x: .Top = y: .Width = w: .Height = h
        .Font.Size = 9
...
Private Sub AddFormButton(dsgn As Object, n As String, cap As String, _
    x As Long, y As Long, w As Long, h As Long)
    Dim ctl As Object
    Set ctl = dsgn.Controls.Add("Forms.CommandButton.1")
    With ctl
        .Name = n: .Caption = cap
        .Left = x: .Top = y: .Width = w: .Height = h
        .Font.Size = 9
...
Private Sub AddFormCheckBox(dsgn As Object, n As String, cap As String, _
    x As Long, y As Long, w As Long, h As Long)
    Dim ctl As Object
    Set ctl = dsgn.Controls.Add("Forms.CheckBox.1")
    With ctl
        .Name = n: .Caption = cap
        .Left = x: .Top = y: .Width = w: .Height = h
        .Font.Size = 9
